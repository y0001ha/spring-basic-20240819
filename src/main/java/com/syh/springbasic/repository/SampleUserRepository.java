package com.syh.springbasic.repository;
...
public interface SampleUserRepository
    ...
    // - findBy : 필드명을 기준으로 모든 컬럼을 조회할 때 사용, findBy 뒤에 필드명을 붙여서 작성,
    // 필드명의 첫글자는 대문자이어야함
    List<SampleUserEntity> findByName(String name); // 조회결과 없으면 빈리스트

    SampleUserEntity findByTelNumber(String telNumber); // 조회결과 없으면 null
    ...
    // - OrderBy: 정렬을 사용할 필드를 짖어하여 Desc, Asc 지정
